...
const WhatsAppFloating = () => {
  // PERBAIKAN: Jangan gunakan tanda "=" tambahan di sini
  const whatsappNumber = import.meta.env.VITE_WHATSAPP_NUMBER || "6285775355771"; 
  
  const handleChat = () => {
    const message = "Halo D'Keys Villa, saya ingin menanyakan ketersediaan kamar.";
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.9 }}
      onClick={handleChat}
      className="fixed bottom-8 right-8 z-50 bg-emerald-500 text-white p-4 rounded-full shadow-lg flex items-center gap-3"
    >
      <span className="font-semibold pl-2">Chat Admin</span>
